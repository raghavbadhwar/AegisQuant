...
**IN PROGRESS.** The candidate includes PIT universe/factors/events/regimes/features, eight portfolio models, isolated attributed pods, abstention-as-cash, the six-way governed evaluation, hash-bound four-pod demo mandate, `aegis-cycle-v2` master trace, and frozen no-network CLI fixtures. Local pre-audit evidence: Ruff format/check passed, strict mypy passed over 114 source files, and 200 tests passed. Criteria 7-9 remain `IN PROGRESS` until a clean immutable commit receives independent adversarial and Ponytail acceptance; this section is not a release claim.
